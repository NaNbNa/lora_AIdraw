...
pretrained_model="./sd-models/model.ckpt" # base model path | 底模路径-----------------------------------一般迁移好后不做修改-----------

is_v2_model=0                             # SD2.0 model | SD2.0模型 2.0模型下 clip_skip 默认无效
parameterization=0                        # parameterization | 参数化 本参数需要和 V2 参数同步使用 实验性功能

train_data_dir="./train/nikke"              # train dataset path | 训练数据集路径--------------------------------按需设置---------------
...
network_dim=64                 # network dim | 常用 4~128，不是越大越好-------------------------------------可以设为64，越大训练难度越高，太小效果过不好-----------------

network_alpha=32               # network alpha | 常用与 network_dim 相同的值或者采用较小的值，如 network_dim的一半 防止下溢。默认值为 1，使用较小的 alpha 需要提升学习率。

# Train related params | 训练相关参数

resolution="512,512"  # image resolution w,h. 图片分辨率，宽,高。支持非正方形，但必须是 64 倍数。----------------------------------与图片预处理的相同配置------------

batch_size=1          # batch size -------------------------------------从1填到3，看显存能力

max_train_epoches=20  # max train epoches | 最大训练 epoch----------------------------------建议15-20，看图片数量
...
lr="1e-4" # learning rate | 学习率，在分别设置下方 U-Net 和 文本编码器 的学习率时，该参数失效------------------------------------学习率-----------
...
output_name="nikke_1"           # output model name | 模型保存名称-------------------------------------------自定义名称------------------------------------------

save_model_as="safetensors" # model save ext | 模型保存格式 ckpt, pt, safetensors--------------------------------------------------建议最后一个格式
